...
- [ ] Suporte a **múltiplos usuários** e operações concorrentes com modelo de permissões coerente.
...
- [ ] **Arquitetura moderna**: microsserviços, **mensageria** e **processamento assíncrono** explicitamente adotados no desenho.

### RC §2 Requisitos funcionais

#### RC §2.1 Gestão de usuários

- [ ] **Cadastro** de usuários (fluxo e validação no back-end).
- [ ] **Edição** de usuários (quem pode alterar o quê).
- [ ] **Exclusão** — física apenas onde política permitir; **desativação** como padrão para preservar histórico.
- [ ] **Controle de perfis e permissões (RBAC)** — papéis e, se aplicável, matriz por módulo/ação.
- [ ] **Autenticação segura** — **JWT** e/ou **OAuth2** (conforme decisão da Fase 0).
- [ ] **Recuperação de senha** — fluxo seguro e assíncrono.
- [ ] **Controle de sessão** — timeout, revogação, opcional listagem/encerramento de sessões.

#### RC §2.2 Controle de acesso

- [ ] **Autorização baseada em papéis** em todas as rotas/ações sensíveis.
- [ ] **Permissões por módulo** (incidentes, requisições, relatórios, configurações, etc.).
- [ ] **Logs de acesso** — autenticação, falhas, acessos a recursos sensíveis, consulta administrativa.
...
- [ ] **Proteção contra SQL Injection** — consultas parametrizadas/ORM adequado, revisão de SQL dinâmico.
...
- [ ] **Otimização de consultas** — índices, EXPLAIN em relatórios pesados, evitar N+1.
...
- [ ] **Arquitetura desacoplada** — front separado, microsserviços, eventos assíncronos.
...
- [ ] **Health checks** — por serviço, consumidos por orquestrador ou monitor.
...
- [ ] **APIs padronizadas RESTful** — recursos, verbos HTTP, códigos de status consistentes.
- [ ] **Comunicação via JSON** — payloads e erros em JSON uniforme (`@pgic/shared`).
...
- [ ] Sistema **dividido em serviços independentes**, cada um com **domínio específico** (usuários, incidentes, relatórios, etc.).
- [ ] **Escalabilidade independente** por serviço (réplicas diferentes conforme carga).
- [ ] **Deploy isolado** — pipeline por artefato/serviço sem redeploy monolítico obrigatório.
- [ ] **Melhor manutenção** — fronteiras claras, ownership por time ou módulo.
- [ ] **Maior resiliência** — falha parcial não derruba todo o sistema se isolamento estiver correto.

#### RC §4.1 — cada microsserviço deve

- [ ] **Responsabilidade única** por serviço (evitar “god service”).
- [ ] Ser **stateless** — estado em BD/cache/broker, não em memória local irrecuperável.
- [ ] **Expor API REST própria** documentada (Swagger/OpenAPI).
- [ ] **Possuir banco isolado quando necessário** — padrão um schema/DB por serviço na PGIC (PostgreSQL por serviço).

### RC §5 Separação entre front-end e back-end

#### RC §5.1 Comunicação via API RESTful

- [ ] **Front-end desacoplado** — pacote `frontend` (e `bff` se usado) sem regra de negócio exclusiva no cliente.
- [ ] **Back-end expõe APIs REST** — microsserviços atrás do gateway.
- [ ] **HTTP/HTTPS** — TLS obrigatório fora do localhost.
- [ ] **Uso de JSON** em request/response.

#### RC §5 — benefícios previstos pelo normativo (verificação de projeto)

- [ ] **Independência tecnológica** — possibilidade de trocar stack do front sem reescrever domínio.
- [ ] **Escalabilidade separada** — CDN/front vs APIs vs workers.
- [ ] **Reutilização da API** — mobile futuro, integrações parceiros, scripts.

### RC §6 Banco de dados

#### RC §6.1 Banco relacional

- [ ] Uso de **PostgreSQL** (ou MySQL/similar — na PGIC: **PostgreSQL** conforme README).
- [ ] **Integridade referencial** — FKs e constraints onde fizer sentido.
- [ ] **Transações ACID** para operações que alteram múltiplas linhas de forma atômica.
- [ ] Modelagem para **dados estruturados** de negócio (incidentes, usuários, SLAs).
...
- [ ] **Relacional para dados críticos** de negócio e consistência forte.
...
- [ ] **Comunicação entre microsserviços** — eventos de domínio, comandos assíncronos.
- [ ] **Filas de tarefas** — relatórios, e-mails, replicação de dados.
- [ ] **Garantia de entrega** — filas duráveis, confirmações de publicação/consumo, outbox onde necessário.

#### RC §7 — benefícios (checklist de adoção real)

- [ ] **Desacoplamento** entre produtores e consumidores de eventos.
- [ ] **Melhor escalabilidade** — workers paralelos por fila.
...
- [ ] **Filas de processamento** — RabbitMQ com routing adequado.
- [ ] **Eventos de domínio** — payloads versionados; contratos entre serviços.
...
- [ ] **Processamento de pagamentos** — *não aplicável à PGIC por padrão*; marcar N/A ou substituir por **cobrança interna de SLA**/integração financeira se existir.
...
- [ ] **Docker** — imagens por serviço ou multi-stage conforme repositório.
- [ ] **Isolamento de serviços** — rede/volumes/containers separados no Compose e em produção.
...
- [ ] **Testes unitários** — domínio e use cases.
- [ ] **Testes de integração** — API + persistência + mensageria em ambiente controlado.
...
- [ ] **Controle de versão (Git)** — fluxo de branches documentado.
- [ ] **Code review obrigatório** antes de merge na principal.
- [ ] **Documentação técnica atualizada** — README, docs, Swagger, runbooks.
...
- [ ] **Arquitetura de microsserviços** adotada e documentada.
- [ ] **Banco relacional e não relacional** em uso conforme estratégia (Postgres + Redis e/ou document store).
- [ ] **Comunicação assíncrona com mensageria** (RabbitMQ) para eventos/tarefas críticas.
- [ ] **Separação front-end e back-end via API REST** com JSON.
...
- [ ] Manter **uma página** com links para: RequisitosCorp, AnaliseRequisitos, MICROSERVICES_LIST, README, este checklist.
...
- [ ] Executar `pnpm docker:up` (ou o script documentado no README) e confirmar que **PostgreSQL**, **Redis**, **RabbitMQ** e **Nginx** sobem sem erro.
- [ ] Verificar portas expostas (gateway em **8080** conforme README, serviços nas portas documentadas).
- [ ] Acessar painel do RabbitMQ (se habilitado no compose) e confirmar login.
- [ ] Confirmar que o Postgres aceita conexão com a URL usada nos serviços.
...
- [ ] Via navegador ou `curl`, chamar **http://localhost:8080** com prefixos `/identity/`, `/request/`, `/api-docs/` conforme documentação.
- [ ] Confirmar que paths sem barra final ou com barra se comportam como esperado (padronizar no Nginx).
- [ ] Registrar no README interno qualquer desvio (porta diferente, TLS local com mkcert, etc.).

### Balanço de saída da Fase 1

- [ ] **Compose sobe** de forma repetível após `docker compose down` + `up`.
- [ ] **Migrations aplicadas** nos serviços em uso.
- [ ] **Gateway responde** e pelo menos um endpoint de health por serviço principal funciona através do proxy.
...
- [ ] Revisar exportações de **DTOs de erro** (`ErrorResponseDto`) e garantir que todos os serviços usam o mesmo formato em falhas.
- [ ] Revisar **validação** (schemas Zod ou equivalente) e reuso de fragmentos comuns (nome, e-mail, paginação).
- [ ] Definir convenção de **paginação** e filtros nas APIs (`limit`, `cursor`, `page`) e documentar no api-docs.
- [ ] Centralizar constantes de **mensageria** (nomes de exchanges, routing keys) quando aplicável, evitando strings mágicas espalhadas.
...
- [ ] Novo endpoint em qualquer serviço segue o **mesmo padrão de erro/validação**.
- [ ] Documentação Swagger reflete contratos compartilhados onde couber.
...
1. [ ] Modelar **usuário** com campos obrigatórios da AnaliseRequisitos (nome, e-mail único, login único, senha hash, perfil, status).
2. [ ] Modelar **perfis/papéis** e relação usuário–perfil; prever **permissões por módulo** ou matriz configurável (RF-2.2).
...
1. [ ] Implementar **cadastro individual** com validação forte no back-end (não confiar no front).
2. [ ] Implementar **edição** com regras: quem pode alterar o quê (próprio usuário vs administrador).
3. [ ] Implementar **desativação (soft delete)** como padrão (RF-1.3); exclusão física apenas com processo e checagem de integridade.
...
1. [ ] Fluxo **login** com emissão de **access token** e **refresh token** (se adotado).
2. [ ] Armazenar senha com **Argon2/bcrypt**; política de complexidade configurável.
3. [ ] Se OAuth2/OIDC: registrar fluxo de callback, escopos, mapeamento de grupos do IdP para perfis internos.
4. [ ] Documentar no Swagger como enviar o token (header `Authorization: Bearer`).

### 3.4 Recuperação de senha (RF-1.6)

**Passos:**

1. [ ] Endpoint “solicitar reset” que não vaza existência de conta (mensagem genérica).
2. [ ] Gerar token de uso único com expiração; persistir hash do token, não o token em claro.
3. [ ] Enfileirar envio de e-mail (**processamento assíncrono**).
4. [ ] **Rate limiting** por IP/e-mail para evitar abuso.

### 3.5 Sessão e revogação (RF-1.7)

**Passos:**

1. [ ] Definir onde a sessão vive: claims no JWT apenas; ou sessão server-side em **Redis** com lista de sessões ativas.
2. [ ] Implementar **logout** que invalida refresh/sessão.
3. [ ] Ao **desativar usuário**, revogar tokens/sessões imediatamente.
4. [ ] Opcional: “encerrar outras sessões” e listagem de sessões por usuário para administrador.

### 3.6 Autorização e logs de acesso (RF-2.1, RF-2.2, RF-2.3)

**Passos:**

1. [ ] Middleware de **autorização** por rota ou por annotação interna (facade única).
2. [ ] Matriz **módulo × ação × escopo** persistida e cacheada com invalidação segura.
3. [ ] Registrar **login sucesso/falha**, **logout**, tentativas bloqueadas; armazenar IP, user-agent, timestamp.
4. [ ] Painel ou API admin para consultar logs de acesso com filtro por período e usuário.

### 3.7 Eventos de domínio (integração com outros serviços)

- [ ] Implementar **Outbox** para `user.created` (ou evento equivalente) conforme padrão documentado em MICROSERVICES_LIST.
- [ ] Operar **relay** que lê outbox e publica no RabbitMQ com confirmação.
- [ ] Monitorar filas mortas ou backlog do relay.

### Balanço de saída da Fase 3

- [ ] Fluxo completo: **cadastro → login → acesso a endpoint protegido → logout**.
- [ ] Tentativa de acesso sem permissão retorna **403** padronizado.
- [ ] **Logs de acesso** consultáveis e evento de usuário propagado para serviços que replicam dados de exibição.
...
1. [ ] Modelar **item de catálogo**: nome, descrição, categoria, equipe responsável, SLA padrão, formulário dinâmico (JSON schema ou tabela de campos).
2. [ ] CRUD administrativo do catálogo com RBAC (apenas perfis autorizados).
...
1. [ ] Modelar **requisição** com estados: Rascunho → Submetida → Em Aprovação → Aprovada/Rejeitada → Em Atendimento → Concluída/Cancelada (ajustar nomes ao seu domínio).
...
- [ ] Garantir fila `request.user_created` (ou nome acordado) ativa.
- [ ] Persistir **replicated_users** para exibir nome do solicitante sem RPC síncrono ao identity em toda listagem.
...
- [ ] Usuário final consegue **escolher item do catálogo**, enviar requisição e acompanhar estado.
...
3. [ ] Para usuário final, permitir **abertura mínima**; analista completa campos adicionais.
...
1. [ ] Cadastro de **níveis de criticidade** e impacto no negócio.
2. [ ] Catálogo de **serviços afetados** com vínculo a **equipes**.
3. [ ] Regra inicial de atribuição por serviço; permitir **reatribuição** com auditoria.

### 5.4 Workflow de estados (RF-5.3)

**Passos:**

1. [ ] Definir grafo de estados e **transições permitidas** por papel.
2. [ ] Impedir transições ilegais no domínio (não só na UI).
3. [ ] Em toda transição, opcionalmente exigir **comentário** (boa prática para auditoria).
...
- [ ] Publicar via outbox (recomendado): `incident.created`, `incident.status_changed`, `incident.assigned`.
- [ ] Definir contrato JSON de cada evento (versionado).

### Balanço de saída da Fase 5

- [ ] Incidente percorre **ciclo completo** com histórico de estados.
...
1. [ ] CRUD de **problema** com status e responsável.
2. [ ] Vincular **N incidentes** a um problema; navegação bidirecional na UI/API.
3. [ ] Campos de **causa raiz** e **plano de ação** (itens com responsável e prazo se desejado).

### 6.2 Mudanças (RF-7.3)

**Passos:**

1. [ ] Modelar mudança com tipo (padrão, normal, emergencial), **risco**, **janela** de execução, **rollback**.
...
4. [ ] Vincular mudança a **incidentes/problemas** motivadores.

### 6.3 Eventos

- [ ] Publicar `problem.created`, `change.created` para auditoria e notificações futuras.
...
1. [ ] Entidades: política de SLA, condições (tipo de chamado, criticidade, serviço, cliente), tempos de resposta e resolução.
...
1. [ ] Configurar **dias úteis**, **feriados**, **janela diária** (ex.: 08:00–18:00).
...
1. [ ] Definir esquema imutável-append-only: quem, quando, entidade, ação, valores anterior/novo, correlation id.
2. [ ] Consumir eventos dos demais serviços ou receber POST dedicado **autenticado serviço-a-serviço**.
3. [ ] API de consulta por **entidade** (ex.: incidente #123) e por **usuário/período**.
...
2. [ ] **Proteção contra SQL Injection** — uso exclusivo de consultas parametrizadas via ORM (Prisma); proibir concatenação de SQL com entrada do usuário; revisar relatórios ou buscas dinâmicas.
...
3. [ ] **Otimização de consultas** — índices alinhados a filtros reais; `EXPLAIN (ANALYZE)` em relatórios pesados; paginação obrigatória em listagens potencialmente grandes.
...
3. [ ] **Arquitetura desacoplada** — front independente do deploy do back; comunicação apenas por HTTP/JSON e eventos; evitar “mega chamada” síncrona encadeando todos os microsserviços num único request crítico.
...
3. [ ] **Health checks** — endpoints `/health` (vivo) e `/ready` (pronto para tráfego, incluindo DB/cache quando aplicável) para orquestrador.
...
1. [ ] **APIs padronizadas RESTful** — recursos nomeados, verbos HTTP corretos, códigos de status consistentes com `@pgic/shared`.
2. [ ] **Comunicação via JSON** — request/response e erros em JSON; charset UTF-8.
...
- [ ] **Isolamento de serviços** — redes e volumes no Compose; em produção, namespaces/projects separados conforme política.
...
- [ ] **Unitários:** domínio puro (SLA tempo útil, transições de estado, validações).
- [ ] **Integração:** API + DB + fila em ambiente efêmero ou dockerizado nos testes.
...
1. [ ] **Controle de versão (Git)** — fluxo de branches (Git Flow, trunk-based ou equivalente) documentado; `main`/`master` protegida.
...
3. [ ] **Documentação técnica atualizada** — README, docs em `/docs`, Swagger (`api-docs`), runbooks de deploy e incidente.
