...
const port = parseInt(process.env.REPORTING_SERVICE_PORT ?? "3010", 10);
...
if (isProduction && !process.env.REPORTING_DATABASE_URL) {
  logger.error("REPORTING_DATABASE_URL must be set in production");
  process.exit(1);
}
...
const baseUrl = process.env.REPORTING_SERVICE_URL ?? `http://localhost:${port}`;
...
async function bootstrap() {
  ...
  const server: Server = app.listen(port, () => {
    logger.info(`Reporting service listening on http://localhost:${port}`);
  });
