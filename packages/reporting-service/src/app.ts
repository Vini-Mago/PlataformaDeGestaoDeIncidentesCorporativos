...
export function createApp(
  ...
  app.use(createMetricsMiddleware("reporting-service"));
  ...
  if (options.baseUrl) {
    const openApiSpec = createReportingOpenApi(options.baseUrl);
    app.get("/api-docs.json", (_req, res) => res.json(openApiSpec));
    app.use(
      "/api-docs",
      swaggerUi.serve,
      swaggerUi.setup(openApiSpec, { customSiteTitle: "Reporting Service API" })
    );
  }

  app.use("/api", container.routes);
  app.get("/health", createHealthHandler("reporting-service"));
  app.get("/metrics", createMetricsHandler("reporting-service", { token: options.metricsToken }));
