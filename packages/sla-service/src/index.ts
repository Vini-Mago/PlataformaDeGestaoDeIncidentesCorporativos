...
const port = parseInt(process.env.SLA_SERVICE_PORT ?? "3006", 10);
if (!Number.isInteger(port) || port < 1 || port > 65535) {
  logger.error("SLA_SERVICE_PORT must be a valid port (1-65535)");
  process.exit(1);
}

const isProduction = process.env.NODE_ENV === "production";
if (isProduction && !process.env.SLA_DATABASE_URL) {
  logger.error("SLA_DATABASE_URL must be set in production");
  process.exit(1);
}

const databaseUrl = isProduction
  ? process.env.SLA_DATABASE_URL!
  : (process.env.SLA_DATABASE_URL ?? "postgresql://pgic:pgic@localhost:5432/sla_service");
...
async function bootstrap() {
  ...
  const server: Server = app.listen(port, () => {
    logger.info(`SLA service listening on http://localhost:${port}`);
  });
...
bootstrap().catch((err) => {
  logger.error({ err }, "Failed to start sla-service");
  process.exit(1);
});
