...
import { createSlaOpenApi } from "./openapi";
import {
  requestIdMiddleware,
...
export interface SlaAppContainer {
...
export function createApp(
  container: SlaAppContainer,
  options: CreateAppOptions = {}
): Express {
  const app = express();
  app.use(requestIdMiddleware);
  app.use(requestLoggingMiddleware);
  app.use(createMetricsMiddleware("sla-service"));
  ...
  if (options.baseUrl) {
    const openApiSpec = createSlaOpenApi(options.baseUrl);
    app.get("/api-docs.json", (_req, res) => res.json(openApiSpec));
    app.use(
      "/api-docs",
      swaggerUi.serve,
      swaggerUi.setup(openApiSpec, { customSiteTitle: "SLA Service API" })
    );
  }
