...
const port = parseInt(process.env.INCIDENT_SERVICE_PORT ?? "3004", 10);
if (!Number.isInteger(port) || port < 1 || port > 65535) {
  logger.error("INCIDENT_SERVICE_PORT must be a valid port (1-65535)");
  process.exit(1);
}
...
async function connectRabbitMQWithRetry(connect: () => Promise<void>): Promise<boolean> {
  for (let attempt = 1; attempt <= rabbitmqConnectRetries; attempt += 1) {
    try {
      await connect();
      if (attempt > 1) {
        logger.info({ attempt }, "incident-service: RabbitMQ connected after retry");
      }
      return true;
    } catch (err) {
      ...
      if (isLastAttempt) {
        ...
        logger.warn(
          ...
          "incident-service: RabbitMQ unavailable in development; continuing with messaging disabled"
        );
        return false;
      }
      logger.warn(
        { err, attempt, retryInMs: rabbitmqConnectRetryDelayMs },
        "incident-service: RabbitMQ connection failed; retrying"
      );
...
async function bootstrap() {
  ...
  const server: Server = app.listen(port, () => {
    logger.info(`Incident service listening on http://localhost:${port}`);
  });
