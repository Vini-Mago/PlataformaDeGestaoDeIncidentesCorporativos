...
import { createProblemChangeOpenApi } from "./openapi";
import {
  requestIdMiddleware,
...
export interface ProblemChangeAppContainer {
...
export function createApp(
  container: ProblemChangeAppContainer,
  ...
  if (options.baseUrl) {
    const openApiSpec = createProblemChangeOpenApi(options.baseUrl);
    app.get("/api-docs.json", (_req, res) => res.json(openApiSpec));
    app.use(
      "/api-docs",
      swaggerUi.serve,
      swaggerUi.setup(openApiSpec, { customSiteTitle: "Problem-change Service API" })
    );
  }

  app.use("/api", container.routes);
  app.get("/health", createHealthHandler("problem-change-service"));
  app.get("/metrics", createMetricsHandler("problem-change-service", { token: options.metricsToken }));
