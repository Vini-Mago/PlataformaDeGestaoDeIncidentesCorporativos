...
export function createApp(
  ...
  app.use(createMetricsMiddleware("notification-service"));
  ...
  if (options.baseUrl) {
    const openApiSpec = createNotificationOpenApi(options.baseUrl);
    app.get("/api-docs.json", (_req, res) => res.json(openApiSpec));
    app.use(
      "/api-docs",
      swaggerUi.serve,
      swaggerUi.setup(openApiSpec, { customSiteTitle: "Notification Service API" })
    );
  }

  app.use("/api", container.routes);
  app.get("/health", createHealthHandler("notification-service"));
  app.get("/metrics", createMetricsHandler("notification-service", { token: options.metricsToken }));
